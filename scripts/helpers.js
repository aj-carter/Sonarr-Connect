...
function RequestShowList(success,profileName){
	//well cache later
    var apiUrl = getApiUrl(profileName);
	var apiKey = settings.getItem("sb_api_key"); //add profile later via function getAPIKey
    request.open("GET", apiURL+'series', true);
	request.setRequestHeader("x-api-key", apiKey);
    var data = request.send(null);
	// i think that will do it; the real question is how we find out what got put into data??
	// to ensure we dont have any malformed json....
	log(data, "REG", INFO);
	success=data;
}
/**
...
function sendUpdatedyupRespaonseRespaonse(request,succescallback){
